    ...
    {#each items as item (item.id)}
      ...
      <li>
        <button
          ...
          class="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left transition focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-focus active:translate-y-[1px] {item.id === selectedId ? 'bg-accent' : 'hover:bg-altrow'}"
        >
          <Icon
            size={17}
            strokeWidth={2}
