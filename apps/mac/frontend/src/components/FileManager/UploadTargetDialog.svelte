...
{#if open}
  ...
  <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 p-4" transition:fade={{ duration: 150 }} onclick={(e) => { if (e.target === e.currentTarget) onClose(); }} onkeydown={onKey} role="presentation">
    <div role="dialog" aria-modal="true" tabindex="-1" aria-label="Upload to phone home folder" transition:scale={{ duration: 180, start: 0.96, opacity: 0 }} class="w-full max-w-[420px] rounded-[12px] border border-separator bg-control p-4 shadow-xl">
      ...
      <div class="mt-4 flex justify-end gap-2">
        <button type="button" onclick={onClose} disabled={busy} class="h-7 rounded-md border border-separator bg-window px-3 text-[13px] text-label transition hover:bg-altrow active:translate-y-[1px] disabled:opacity-50">Cancel</button>
        <button type="button" onclick={() => onHomeAnyway(remember)} disabled={busy} class="h-7 rounded-md border border-separator bg-window px-3 text-[13px] text-label transition hover:bg-altrow active:translate-y-[1px] disabled:opacity-50">Upload here anyway</button>
