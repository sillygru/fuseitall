...
<script lang="ts">
  ...
  let selected = $state(suggested || 'Download');
...
{#if open}
  <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 p-4" transition:fade={{ duration: 150 }} onclick={onClose} onkeydown={onKey} role="presentation">
    <div role="dialog" aria-modal="true" aria-label="Upload to phone home folder" transition:scale={{ duration: 180, start: 0.96, opacity: 0 }} class="w-full max-w-[420px] rounded-[12px] border border-separator bg-control p-4 shadow-xl" onclick={(e) => e.stopPropagation()}>
      ...
      <fieldset class="mt-3 max-h-[180px] overflow-auto rounded-md border border-separator bg-window p-1" aria-label="Phone folder">
        {#each folders as f (f)}
      ...
      <div class="mt-4 flex justify-end gap-2">
        <button type="button" onclick={onClose} disabled={busy} class="h-7 rounded-md border border-separator bg-window px-3 text-[13px] text-label transition hover:bg-altrow focus-visible:outline-2 focus-visible:outline-focus active:translate-y-[1px] disabled:opacity-50">Cancel</button>
        <button type="button" onclick={() => onHomeAnyway(remember)} disabled={busy} class="h-7 rounded-md border border-separator bg-window px-3 text-[13px] text-label transition hover:bg-altrow focus-visible:outline-2 focus-visible:outline-focus active:translate-y-[1px] disabled:opacity-50">Upload here anyway</button>
        <button type="button" onclick={() => onPick(selected || 'Download', remember)} disabled={busy || !selected} class="h-7 rounded-md bg-accent px-3 text-[13px] font-medium text-accent-text transition hover:brightness-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus active:translate-y-[1px] disabled:opacity-50">Use {selected || 'Download'}</button>
      </div>
    </div>
  </div>
{/if}
