...
{#if renameTarget}
  ...
  <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 p-4" transition:fade={{ duration: 150 }} onclick={(e) => { if (e.target === e.currentTarget) onRenameClose(); }} onkeydown={(e)=> e.key==='Escape' && onRenameClose()} role="presentation">
    <div role="dialog" aria-modal="true" tabindex="-1" aria-label="Rename" transition:scale={{ duration: 180, start: 0.96, opacity: 0 }} class="w-full max-w-[380px] rounded-[12px] bg-control p-4 shadow-xl">
      ...
      <input value={renameValue} oninput={(e) => onRenameValue((e.currentTarget as HTMLInputElement).value)} placeholder="New name" aria-label="New name" class="mt-3 h-8 w-full rounded-md bg-window px-2 text-[13px]" onkeydown={(e)=> e.key==='Enter' && onRenameConfirm()} />
      <div class="mt-4 flex justify-end gap-2">
        <button type="button" onclick={onRenameClose} class="h-7 rounded-md bg-window px-3 text-[13px] text-label transition hover:bg-altrow active:translate-y-[1px]">Cancel</button>
...
{#if deleteTarget}
  ...
  <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 p-4" transition:fade={{ duration: 150 }} onclick={(e) => { if (e.target === e.currentTarget) onDeleteClose(); }} onkeydown={(e)=> e.key==='Escape' && onDeleteClose()} role="presentation">
    <div role="dialog" aria-modal="true" tabindex="-1" aria-label="Delete file" transition:scale={{ duration: 180, start: 0.96, opacity: 0 }} class="w-full max-w-[380px] rounded-[12px] bg-control p-4 shadow-xl">
      ...
      <div class="mt-4 flex justify-end gap-2">
        <button type="button" onclick={onDeleteClose} class="h-7 rounded-md bg-window px-3 text-[13px] text-label transition hover:bg-altrow active:translate-y-[1px]">Keep</button>
